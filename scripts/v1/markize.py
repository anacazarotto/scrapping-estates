...
# Mapeados a partir do markize.json (retorno real da API)
ID_QUARTOS = 3  # Dormitório(s)
ID_QUARTOS_FALLBACK = 5  # Sendo suite(s) — fallback quando id 3 não estiver presente
ID_BANHEIROS = 6  # Banheiro(s)
ID_VAGAS = 4  # Vaga(s) Garagem
ID_VAGAS_2 = 15  # Vaga(s) Estacionamento (fallback)
ID_AREA_PRIVADA = 9  # Área Útil (Aprox.)
ID_AREA_TOTAL = 27  # Área Total (Aprox.)
...
# encapsula o main para não executar ao importar
def main():
    global first_result
    while True:
        payload_inner = {
        ...
        for item in results:
            ...
            # Área útil (id 9) e área total (id 27)
            area_privada = get_carac_valor(
                caracteristicas, ID_AREA_PRIVADA
            )  # Área Útil
            area_total = get_carac_valor(caracteristicas, ID_AREA_TOTAL)  # Área Total
            ...
            for c in caracteristicas:
                if c.get("idCaracteristica") == ID_QUARTOS:
                    ...
                    break
            # fallback: usa id 5 se id 3 não foi encontrado
            if not raw_quartos:
                for c in caracteristicas:
            ...
            raw_vagas = None
            for c in caracteristicas:
                if c.get("idCaracteristica") in (ID_VAGAS, ID_VAGAS_2):
                    raw_vagas = (
                        c.get("valor")
                        or c.get("valorFormatado")
                        or c.get("valorText")
            ...
            print(
                f"    Área: {area_privada} m², Preço/m²: R$ {preco_m2}, Quartos: {quartos}, Banheiros: {banheiros}, Vagas: {vagas}"
            )
            sleep(1)
