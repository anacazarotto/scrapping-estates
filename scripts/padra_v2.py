...
class PadraScraper(BaseScraper):
    ...
    ID_QUARTOS = 89
    ID_QUARTOS_FALLBACK = 103
    ID_BANHEIROS = 84
    ID_VAGAS = 104
    ID_AREA_PRIVADA = 124
    ID_AREA_TERRENO = 107
    ID_AREA_TOTAL = 123
    ID_AREA_TOTAL_EDIF = 109
    ID_AREA_UNIDADE = 106
    ...
    def scrape_imovel(self, codigo, item_lista):
        ...
        area_total = (
            self.get_carac_valor(caracteristicas, self.ID_AREA_TOTAL_EDIF)
            or self.get_carac_valor(caracteristicas, self.ID_AREA_TERRENO)
            or self.get_carac_valor(caracteristicas, self.ID_AREA_TOTAL)
            or self.get_carac_valor(caracteristicas, self.ID_AREA_UNIDADE)
        )
        ...
        raw_vagas = self._get_raw_valor_from_carac(caracteristicas, {self.ID_VAGAS})
        try:
    ...
    def run(self):
        print("[PADRA_V2] Iniciando coleta")
        first_result = 0
        tipos_ignorados_norm = {self.normalize_str(t) for t in self.TIPOS_IGNORADOS}

        while True:
            print(f"[PADRA_V2] Buscando pagina com firstResult={first_result}")
            results = self.pegar_links(first_result)

            if not results:
                print(
                    f"[PADRA_V2] Nenhum resultado em firstResult={first_result}. Encerrando coleta."
                )
                break

            total_results = len(results)
            print(
                f"[PADRA_V2] firstResult={first_result}: {total_results} imoveis encontrados."
            )
            ...
            for idx, item in enumerate(results, start=1):
                ...
                if not codigo:
                    print(
                        f"[PADRA_V2] ({idx}/{total_results}) Item sem codigo. Ignorado."
                    )
                    continue

                try:
                    print(
                        f"[PADRA_V2] ({idx}/{total_results}) Coletando codigo={codigo}"
                    )
                    dados = self.scrape_imovel(codigo, item)

                    if not dados:
                        print(
                            f"[PADRA_V2] ({idx}/{total_results}) Detalhe nao encontrado para codigo={codigo}"
                        )
                        time.sleep(1)
                        continue
                    ...
                    if tipo_norm in tipos_ignorados_norm:
                        print(
                            f"[PADRA_V2] ({idx}/{total_results}) Ignorado codigo={codigo} tipo={dados.get('tipo_imovel')}"
                        )
                        time.sleep(1)
                        continue
                    ...
                    print(
                        f"[PADRA_V2] ({idx}/{total_results}) Salvo ID={dados.get('id')} tipo={dados.get('tipo_imovel')} bairro={dados.get('bairro')}"
                    )
                    time.sleep(1)
                except Exception as e:
                    print(
                        f"[PADRA_V2] ({idx}/{total_results}) Erro ao processar codigo={codigo}: {e}"
                    )

            if total_results < self.MAX_RESULTS:
                print(
                    "[PADRA_V2] Ultima pagina identificada pela quantidade de resultados."
                )
                break
        ...
        print("[PADRA_V2] Coleta finalizada")


if __name__ == "__main__":
    scraper = PadraScraper()
    try:
        scraper.run()
    finally:
        scraper.close()
